                    ...
                    if (headers.Authorization.Scheme.Equals("Bearer"))
                    {
                        ...
                        ResourceServer server = new ResourceServer(
                                        new StandardAccessTokenAnalyzer(
                                            (RSACryptoServiceProvider)MvcApplication.SigningCertificate.PublicKey.Key,
                                            (RSACryptoServiceProvider)MvcApplication.EncryptionCertificate.PrivateKey
                                        )
                                    );
                        ...
                        if (principal != null && principal.Identity != null && principal.Identity.IsAuthenticated)
                        {
                            ...
                            foreach (string scope in principal.Roles)
                                claims.Add(new Claim("http://mycontacts.development.tomrankin.net/contacts/OAuth20/claims/scope", scope));
